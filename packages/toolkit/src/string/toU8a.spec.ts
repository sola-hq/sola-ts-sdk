...
describe('stringToU8a', (): void => {
  it('decodes to an empty string for undefined/null', (): void => {
    expect(stringToU8a()).toEqual(new Uint8Array());
    expect(stringToU8a(null)).toEqual(new Uint8Array());
  });

  it('encodes the string correctly', (): void => {
    expect(stringToU8a('Привет, мир!')).toEqual(RUSS_HELLO);
  });

  it('encodes the string correctly (String)', (): void => {
    expect(stringToU8a(String('Привет, мир!'))).toEqual(RUSS_HELLO);
  });
