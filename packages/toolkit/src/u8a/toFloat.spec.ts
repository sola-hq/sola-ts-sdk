...
describe('u8aToFloat', (): void => {
  ...
  it('throws on invalid input length', (): void => {
    expect(() => u8aToFloat(new Uint8Array())).toThrow();
  });

  describe('32-bit', (): void => {
    it('correctly decodes +0.0', (): void => {
      expect(u8aToFloat(hexToU8a('0x00000000'))).toEqual(+0.0);
    });

    it('correctly decodes -0.0', (): void => {
      expect(u8aToFloat(hexToU8a('0x00000080'))).toEqual(-0.0);
    });

    it('correctly decodes -0.0 (BE)', (): void => {
      expect(u8aToFloat(hexToU8a('0x80000000'), { isLe: false })).toEqual(-0.0);
    });

    it('decoded NaN', (): void => {
      expect(u8aToFloat(hexToU8a('0x0000c07f'))).toEqual(Number.NaN);
    });

    // https://www.h-schmidt.net/FloatConverter/IEEE754.html
    it('decodes a known number', (): void => {
      expect(u8aToFloat(hexToU8a('0x79e9f642')).toFixed(3)).toEqual('123.456');
    });
  ...
  describe('64-bit', (): void => {
    ...
    it('correctly decodes -0.0 (BE)', (): void => {
      expect(
        u8aToFloat(hexToU8a('0x8000000000000000'), {
          bitLength: 64,
          isLe: false,
        }),
      ).toEqual(-0.0);
