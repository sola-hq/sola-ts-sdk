...
describe('u8aToU8a', (): void => {
  ...
  it('should handle faker generated hex strings', () => {
    ...
    const expectedU8a = new Uint8Array(hexString.slice(2).match(/.{1,2}/g)!.map(byte => parseInt(byte, 16)));
  ...
  it('should handle faker generated number arrays', () => {
    const size = faker.number.int({ min: 0, max: 100 });
    const numberArray = Array.from({ length: size }, () => faker.number.int({ min: 0, max: 255 }));
