 ...
 */
export function u8aToNumber(
  value: Uint8Array,
  { isLe = true, isNegative = false }: ToBnOptions = {},
): number {
  ...
  if (isNegative && count && value[count - 1] & 0x80) {
    switch (count) {
      ...
      case 3:
        return (
          ((value[0] + (value[1] << 8) + (value[2] << 16)) ^ 0x00ff_ffff) * -1 -
          1
        );
  ...
  switch (count) {
    ...
    case 5:
      return (
        ...
        (value[1] << 8) +
        (value[2] << 16) +
        (value[3] + (value[4] << 8)) * 0x1_00_00_00
      );
    ...
    case 6:
      return (
        ...
        (value[1] << 8) +
        (value[2] << 16) +
        (value[3] + (value[4] << 8) + (value[5] << 16)) * 0x1_00_00_00
      );
