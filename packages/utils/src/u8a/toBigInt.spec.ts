...
describe('u8aToBigInt', (): void => {
  ...
  it('should handle faker generated positive Uint8Array', () => {
    ...
    const expectedBigInt = randomBytes.reduce((acc, byte, index) => acc + BigInt(byte) * (1n << BigInt(index * 8)), 0n);
  ...
  it('should handle faker generated negative Uint8Array', () => {
    ...
    const expectedBigInt = u8aToBigInt(randomBytes, { isLe: true, isNegative: true });
    expect(u8aToBigInt(randomBytes, { isLe: true, isNegative: true })).toEqual(expectedBigInt);
  });
