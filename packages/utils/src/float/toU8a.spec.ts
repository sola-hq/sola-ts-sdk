...
describe('floatToU8a', (): void => {
  ...
  describe('conversion tests', (): void => {
    test.each([
      ...
      expect(u8aToHex(floatToU8a(input, { bitLength: bitLength as 32 | 64, isLe }))).toEqual(output);
    });
  });

  it('should handle faker generated float numbers (32-bit)', () => {
    const num = faker.number.float({ min: -100000, max: 100000, fractionDigits: 8 });
  ...
  it('should handle faker generated float numbers (64-bit)', () => {
    const num = faker.number.float({ min: -1000000000, max: 1000000000, fractionDigits: 8 });
  ...
  it('should handle faker generated float numbers with different endianness', () => {
    const num = faker.number.float({ min: -1000, max: 1000, fractionDigits: 8 });
